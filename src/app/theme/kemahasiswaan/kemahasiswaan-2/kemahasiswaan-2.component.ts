...
@Component({
  selector: "app-kemahasiswaan-2",
  templateUrl: "./kemahasiswaan-2.component.html",
  styleUrls: ["./kemahasiswaan-2.component.scss"],
})
export class Kemahasiswaan2Component implements OnInit {
    ...
    this.appService.getFakeDataKemahasiswaan().subscribe(
      (response) => {
        for (const val of response) {
          if (val.students_id === "STD-002") {
            this.listData = val.data;
            for (let i = 0; i < this.listData.length; i++) {
              this.listData[i].total_pendanaan = this.listData[i].total_funding
                .toString()
                .replace(/\B(?=(\d{3})+(?!\d))/g, ".");
            }
            this.chart_A();
